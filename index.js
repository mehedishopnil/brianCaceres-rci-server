...
async function run() {
  ...
  try {
    ...
    const allResortDataCollection = db("allResorts");
    const usersCollection = db("users");
    const allBookingsCollection = db("allBookings");
    const paymentInfoCollection = db("paymentInfo");
...
//     if (result.modifiedCount === 0) {
//       return res.status(404).json({ success: false, message: "User not found or information not updated." });
//     }
